...
public class CookieBox {
	...
	public CookieBox(HttpServletRequest request) {
		//request를 통해 Cookie정보를 얻을 수 있다.
		
		Cookie[] cookies = request.getCookies();
		
		if(cookies!=null && cookies.length>0) {
			for(int i =0; i<cookies.length; i++) {
				//cookiMap에 Cookie 객체를 저장
	...
	public String getValue(String name) throws IOException {
		...
		Cookie cookie = cookieMap.get(name); //Map에 name의 키가 없는 경우 null반환도 가능
	...
	//쿠키 객체를 생성해주는 메소드: 객체를 생성하지않고도 사용할 수 있는 메소드로 정의:static멤버로 정의
	...
	//이름, 값, 경로, 경로, 유지 시간을 가지고 cookie객체를 생성
	...
	//이름, 값, 경로, 경로, 유지 시간, 도메인을 가지고 cookie객체를 생성
		public static Cookie createCookie(String name, String value, 
