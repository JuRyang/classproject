...
--테이블 생성 : creat table table_name
--(
--    column_name dmain [constraint],
--    column_name dmain [constraint],
--    column_name dmain [constraint]
--    .....
--  )

--date도 되지만 Timestamp도 날짜타입 저장하는 것 
--SYSDATE 현재시간
drop table tset_tbl;
...
--테이블의 삭제: 저장공간을 삭제 -> 저장되어 있는 데이터도 모두 삭제 (삭제하면 복구 안됨)
--drop table table_name
drop table tset_tbl;

--TRUCATE 테이블의 모든 (행)로우를 바로 제거  -> 물리적인 적용도 바로 진행 
create table emp0
as
select * from emp;
select*from emp0;
TRUNCATE table emp0; --특별한 경우 아니면 쓸일 없음

--rename 현재 테이블 이름 to  새로운 이름
rename emp0 to new_emp;
select * from tab;
...
--기본 테이블의 컬럼 변경: 새로 정의된 컬럼으로 교체하는 것.
alter table emp01
...
drop table emp03;
create table emp03(
   empno number(4) not null unique, --not null unique 두개의 제약조건을 사용할때 공백으로 띄어 입력
   ename varchar2(10) not null
);
insert into emp03 values(null, 'test1');
insert into emp03 values(1, null);
insert into emp03 values(1, 'test1');
insert into emp03 values(1, 'test2');  --사원번호 중복 무결성제약조건
...
create table emp05(
   empno number(4) primary key,
   ename varchar2(10) not null,
   deptno number(2) REFERENCES dept(deptno) --dept(deptno)외래키 참조 regerneces
);

insert into emp05 values(1,'test',10);
insert into emp05 values(1,'test',60); --부서 번호는 40까지 있기때문에 조건에 안맞음 오류
insert into emp05 values(2,'test',null); -- deptno 40까지, null값도 허용 
...
insert into emp06 values (1,'test',799); --check(sal>=800) 이기 때문에
...
insert into emp09 values(null, null,'job',50);
...
--데이터 삽입 
--insert into table_name(데이터 저장 할 컬럼들) valuse(저장할 데이터들)
...
select * from dept01 where deptno = 200;
...
--부서 번호가 40번인 부서와 동일하게 변경하기 위한 UPDATE 명령문입니다. 
...
--테이블의 행 삭제 
--delect from table_name: 행단위 삭제, where가 없으면 전체 행이 대상
...
--emp10테이블의 데이터 중 부서번호가 10번 부서의 사원들을 삭제
...
--commit 완료 물리적 저장
--rollback -> 마지막으로 물리적 저장이 된 commit상태 
--DDL은 무조건 create commit 저장됨 
