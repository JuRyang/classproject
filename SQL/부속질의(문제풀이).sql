...
--45. 최소급여를 받는 
--사원의 이름,담당업무 및 급여를 표시하시오. 메인쿼리(그룹함수 사용)
--min()
...
select min(avg(sal)) from emp group by job; --평균 급여
...
from emp e1
where sla=(select min(sal)
           from emp e2
           where e2.detno = e1.deptno
           group by e2.deptno
)
;
...
select distinct(deptno) from emp  where ename like '%K%'; --k 들어간 사람
